...
panel at H8. The column's own sub-header is a tick, which is what it meant
before that instruction was added; the instruction wins, because it is the one
addressed to the person filling the form in. On screen it is a tick box, and
the answer is only written **once the row says something else too**: an
unticked box submits nothing, so a row that collected a "No" would stop being
blank and twenty-two untouched rows would print "No" twenty-two times.

**`RE ROOKIE`** is `y` or `n`, which is what the sub-header at C26 says.
...
A blank form generated by this application was diffed against that workbook
cell by cell — **558 cells, style id and value, zero differences; all sixty
merged ranges, all thirteen column runs and every row attribute identical**.
The cell count is asserted as the cheap invariant that keeps it that way.

**The two literal zeros are reproduced, on untouched rows only.** The blank
form Rodeo Houston sends out carries a numeric `0` in the ROOKIE and WAIT LIST
cells of all twenty-five rows — plainly a leftover of theirs, and plainly what
a printed blank RCF looks like today. Rows nobody filled in keep it; rows
somebody filled in carry their answer. "Exactly like the form" is a claim about
the paper, not about the tidiest possible file.

**Everything a person typed is written as a string.** `FormSheet` has no
general numeric cell — `number()` refuses anything but an integer and exists
solely for those zeros — which is what keeps Customer Number 1234567 from
becoming 1234567.0, the same rule `Spreadsheet::open()` enforces coming the
other way.
...
| `B` | `*TYPE` — the code |
| `C` | `ROOKIE` — `y`/`n`, or the blank form's `0` |
...
| `F` | `CHANGE/ADD TITLE` |
| `G` | `PREVIOUS TITLE` |
| `H` | `WAIT LIST` — `Yes`/`No`, or the blank form's `0` |
