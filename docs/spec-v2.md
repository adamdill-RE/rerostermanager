...
So both columns carry `1` or `0` on **every** row, exactly as the blank form
does, and both are tick boxes on the screen. Neither value counts towards
...
This is also the one defect that reached a real Excel. The first version
shipped `styles.xml` alone; Excel resolved `xfComplement i="0"` into a feature
property bag that was not in the package, and opened the form with *"Repaired
Records: Format from /xl/styles.xml part (Styles)"* — the checkboxes gone and
the user told their form was damaged. The rule that came out of it is in §2.5.
...
merged ranges and every row attribute identical**, and both shipped parts
byte-identical inside the package. The cell count is asserted as the cheap
...
solely for the fifty checkbox cells (§2.1), which have to be numeric or Excel
stops drawing the box. That is what keeps Customer Number 1234567 from becoming
1234567.0, the same rule `Spreadsheet::open()` enforces coming the other way.
...
| `B` | `*TYPE` — the code |
| `C` | `ROOKIE` — a **checkbox**, `1` ticked or `0` not, on every row |
...
| `F` | `CHANGE/ADD TITLE` |
| `G` | `PREVIOUS TITLE` |
| `H` | `WAIT LIST` — a **checkbox**, `1` ticked or `0` not, on every row |
